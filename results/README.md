...
판정 모델 Jev를 네 번째 판정자로 넣은 점검은 `jev/` 아래에 따로 둔다(2026-09-23). 판정 파일(`jev/single/*.jsonl`)은 문항 키와 확률만 담고, 구성별 산출물(`jev/panels/`)은 위 두 스크립트의 함수를 그대로 불러 낸 것이다(판정자 목록과 읽는 행만 바꾼다). anon 소스만으로 같은 값이 나온다(raw 대조 일치 확인). 설계와 해석은 [jev/README.md](jev/README.md)에 있다. 이 절의 정본 파일은 바뀌지 않는다.
