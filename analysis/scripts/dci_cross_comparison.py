...
OUT_DIR = Path(r"C:/Users/junto/Downloads/analysis_v8/dci_out/cross")
CATS = ["I_니라_O", "II_니라_X", "III_라_O", "IV_라_X"]

SYSTEM_PROMPT = (
    "한문 고전 번역문의 언어학적 분석 전문가. "
    "아래 증거는 11,961건 전체 코퍼스에서 4범주 동시 grep/awk 등가 연산으로 추출한 횡단 비교 데이터이다. "
...
def run():
    ...
    print(f"  합계: {sum(len(data[c]) for c in CATS)}건")
        ...
        try:
            resp = client.chat.completions.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": (
                        f"## 횡단 비교 질문\n{q}\n\n"
                        f"## DCI 추출 증거 (4범주 동시 전수 검색, 11,961건)\n{evidence}"
                    )},
                ],
            )
