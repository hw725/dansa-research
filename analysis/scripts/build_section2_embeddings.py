"""Section 2 embedding pipeline: merge cleaned 3-model consensus → embed → save.

Input:  results/{gpt5mini,gemini,claude_sonnet}/section2_decision_judgments.csv
        plus supplement_section2_judgments.csv
Output: parallel_data_v2_cleaned.tsv, emb_openai_v2.npy
...
def build_consensus() -> pd.DataFrame:
    def load_model(model: str) -> pd.DataFrame:
        ...
        frames = [
            pd.read_csv(os.path.join(model_dir, "section2_decision_judgments.csv")),
            pd.read_csv(os.path.join(model_dir, "supplement_section2_judgments.csv")),
        ]
        df = pd.concat(frames, ignore_index=True)
        return df.drop_duplicates(subset=MERGE_KEYS, keep="last")

    gpt = load_model("gpt5mini")
    gem = load_model("gemini")
    cla = load_model("claude_sonnet")
...
def main():
    ...
    out_tsv = os.path.join(ROOT, "parallel_data_v2_cleaned.tsv")
