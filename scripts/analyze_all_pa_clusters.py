#!/usr/bin/env python3
"""
PA Cluster 16개 전체에 대한 심층 분석

각 클러스터별로:
1. 크기, 사서 비율, 서종 분포
2. Top 마커 패턴 (left~right)
...
print("=== PA Cluster 결과 로드 ===")
df = pd.read_csv('hyeonto/reports/boundary_function_clusters/pa_clusters_with_features.csv')
...
for cluster_id in sorted(df['cluster_id'].unique()):
    ...
    saseo_ratio = saseo_count / size * 100
...
# JSON 저장
output_path = 'hyeonto/reports/pa_all_clusters_analysis.json'
...
for cluster_id in sorted(df['cluster_id'].unique()):
    cluster_df = df[df['cluster_id'] == cluster_id]
    saseo_count = len(cluster_df[cluster_df['book_category'] == '사서'])
    saseo_ratio = saseo_count / len(cluster_df) * 100
...
for cluster_id, ratio, count, total in saseo_ratios[:5]:
    print(f"  Cluster {cluster_id}: {ratio:.1f}% ({count}/{total})")
