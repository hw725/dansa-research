...
# S2P 출력 (v2 매핑)
pred = pd.read_csv('test_results/s2p_test_mapped_v2.csv')

print(f"  Gold v2: {len(gold)} rows, {gold['문장식별자'].nunique()} 문장")
print(f"  Pred v2: {len(pred)} rows, {pred['원본문장식별자'].nunique()} 문장")
...
# 결과
print("\n" + "="*60)
print("📊 S2P 평가 결과 (v2 매핑, 제한적 커버리지)")
print("="*60)
...
with open('test_results/s2p_eval_v2_limited.json', 'w') as f:
    json.dump(results, f, indent=2)
print(f"\n💾 저장됨: test_results/s2p_eval_v2_limited.json")
