...
권장 실행(도커)
  docker-compose run --rm csp python scripts/train_and_eval_pa_trainonly.py --enable-hard-neg

스모크(빠르게)
  docker-compose run --rm csp python scripts/train_and_eval_pa_trainonly.py --enable-hard-neg --max-steps 50
"""
...
def _run_allow(
    ...
    *,
    ...
    integrity_report.py는 '불일치 항목 존재'를 returncode=2로 표현할 수 있는데,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train-only PA alignment + PA strict + gold eval")

    # train (PA sentence-parallel)
    parser.add_argument(
        "--train-csv",
    ...
    parser.add_argument(
        "--deterministic",
        action="store_true",
        help="가능한 범위에서 결정론적 실행을 강제(재현성 강화). PA 실행에도 전달됨.",
    )

    parser.add_argument(
        "--skip-train",
        action="store_true",
        help="alignment 학습을 건너뛰고 기존 models/dual_encoder_alignment_pa.pt로 PA+평가만 수행",
    )
    ...
    out_pa_csv = out_dir / f"pa_strict_{ts}.csv"
    ...
    # 2) Run PA strict on eval paragraphs
    ...
    # 재현성 옵션: PA 실행에도 전달
    if int(args.seed) != 0:
        pa_cmd.extend(["--seed", str(int(args.seed))])
