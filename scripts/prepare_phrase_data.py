#!/usr/bin/env python3
"""
Phrase 데이터 정규화 및 준비 (phrase 단위 보조 분석용)

`datasets/phrase_full.csv` → `data/phrase_normalized.csv`

【주의】 v3 메인 파이프라인은 sentence 기준이며 `prepare_sentence_dataset.py`를 쓴다.
본 스크립트는 비-종결 현토 분석(연결·가정·양보·의문 등)이 필요해질 때
`classify_premodern_markers.py`의 38범주 분류 입력을 만들기 위한 보조용이다.

【사전 준비】 `datasets/phrase_full.csv`(643K행, 句 단위, .gitignore 처리)는
원본 보관 PC에서 가져와야 한다.
"""
