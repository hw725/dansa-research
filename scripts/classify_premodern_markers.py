"""Phase 4: 전근대 원전 기준 현토 재분류 (phrase 단위 보조 분석용)

본 스크립트는 phrase 단위 데이터(`data/phrase_normalized.csv`)를 입력으로 받아
38개 범주(PREMODERN_TAXONOMY)로 분류한다. 현 v3.1 정량 검증의 메인 입력은
sentence 기준 `data/sentence_normalized.csv`이며, 이 스크립트는 비-종결 현토
보조 분석이 필요할 때만 사용한다.
...
def run_classification():
    ...
    """전체 마커 분류 실행 (구 단위 데이터)"""

    df = pd.read_csv("data/phrase_normalized.csv")

    if 'marker_final' in df.columns:
        marker_series = df['marker_final']
    elif 'marker_normalized' in df.columns:
        marker_series = df['marker_normalized']
    else:
    ...
    marker_counts = marker_series.value_counts().to_dict()
